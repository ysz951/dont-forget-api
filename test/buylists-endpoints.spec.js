...
describe('Lists Endpoints', function() {
  ...
  describe(`POST /api/buylists`, () => {
    beforeEach('insert buylists', () =>
      helpers.seedLists(
        db,
        testUsers,
        testLists,
  ...
  describe(`DELETE /api/buylists/:list_id`, () => {
    context('Given there are comments in the database', () => {
      ...
      it('responds with 204 and removes the comment', () => {
        ...
        const expectedDeleteLists = helpers.makeExpectedBuyLists(testUser, deleteLists);
        return supertest(app)
          .delete(`/api/buylists/${idToRemove}`)
          .set('Authorization', helpers.makeAuthHeader(testUser))
          .expect(204)
          .then(res =>
            supertest(app)
  ...
  describe(`PATCH /api/buylists/:list_id`, () => {
    context('Given there are lists in the database', () => {
      beforeEach('insert lists', () =>
        helpers.seedLists(
          db,
      ...
      it('responds with 204 and updates the list', () => {
        ...
        return supertest(app)
          .patch(`/api/buylists/${idToUpdate}`)
          .set('Authorization', helpers.makeAuthHeader(testUsers[0]))
          .send(updateList)
          .expect(204)
          .then(res =>
            supertest(app)
              .get(`/api/buylists/${idToUpdate}`)
              .set('Authorization', helpers.makeAuthHeader(testUsers[0]))
              .expect(expectedList)
          )
      });
  ...
  describe(`GET /api/buylists/:list_id/items`, () => {
    beforeEach('insert items', () =>
        helpers.seedItems(
        db,
        testUsers,
